...
function LoginChild() {
    ...
    useEffect(() => {
        const fetchChildren = async () => {
            ...
            try {
                const response = await fetch('https://kkuleogi.kro.kr/api/v1/child/login', {
                    method: 'POST',
                    headers: {
                ...
                if (data.success) {
                    ...
                    const childrenDetails = await Promise.all(childrenIds.map(async (id) => {
                        const childResponse = await fetch(`https://kkuleogi.kro.kr/api/v1/child/${id}`, {
                            method: 'GET',
                            headers: {
                                'Authorization': `Bearer ${token}`
                            }
    ...
    const handleSubmit = async (e) => {
        ...
        // API 요청
        const response = await fetch('https://kkuleogi.kro.kr/api/v1/child', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
